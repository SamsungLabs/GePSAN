...
class YouCookII_Orig:
    def __init__(self, args=None, train=True, load_resnet_feats=False, **kwargs):
        logging.info("Using YouCookII original Splits")
        self.sentences, self.ingredients, self.ids = dict(), dict(), dict()
        self.verbs = dict()
        ...
        for i, (c_id, entry) in tqdm(enumerate(annotations.items())):
            ...
            if np.sum(ingredient_arr) > 0 and lenSent > 1:
                video_split = entry['subset']
                
                if self.train and video_split=='validation':
                    continue
                elif (not self.train) and (video_split=='training'):
                    continue

                if c_id not in visual_univl_features_dict:
                    print(f"ignoring recipe {c_id} due to missing visual features\n")
                    continue
